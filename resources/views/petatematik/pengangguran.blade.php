...
<head>
    ...
    <title>Tingkat Pengangguran</title>
...
<header>
    ...
    <script> 
        ...
        var tiles = L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
            maxZoom: 19,
            attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> | <a href="https://bali.bps.go.id/id/statistics-table/2/Mjg4IzI=/persentase-pengangguran-provinsi-bali-menurut-kabupaten-kota.html">BPJS di Provinsi Bali Kab/Kota (Tingkat Pengangguran)</a>'
        }).addTo(map);
        ...
        const regencyData = regencies.map(regency => ({
            ...
            properties: {
                name: regency.name,
                id: regency.id,
                pengangguran: regency.pengangguran,
            },
            geometry: {
        ...
        function getColor(d) {
    return d > 3.300  ? '#3E2A47' :   // Coklat gelap
           d > 2.950  ? '#5D3A4E' :
           d > 2.650  ? '#7F4B61' :
           d > 2.350  ? '#A75C74' :
           d > 2.050  ? '#C67D8D' :
           d > 1.750  ? '#D68A9D' :
           d > 1.450  ? '#E39AA9' :
           d > 1.150  ? '#F0B8B9' :   // Coklat muda
                        '#F7D1D2'; // Coklat yang sangat terang
}

        function style(feature) {
            return {
                fillColor: getColor(feature.properties.pengangguran),
                weight: 2,
                opacity: 1,
                color: 'white',
                dashArray: '3',
        ...
        info.update = function (props) {
            this._div.innerHTML = '<h4>Tingkat Pengangguran di Kab/Kota Bali</h4>' +  (props ?
                '<b>' + props.name + '</b><br />' + props.pengangguran.toLocaleString('id-ID') + ' pengangguran'
                : 'Hover over a state');
        };
        ...
        legend.onAdd = function (map) {
            ...
            var div = L.DomUtil.create('div', 'info legend'),
                grades = [0, 1.153, 1.456, 1.759, 2.052, 2.355, 2.658, 2.951, 3.309],
                labels = [];
