...
<head>
    ...
    <title>Distribusi Pariwisata</title>
...
<header>
    ...
    <script> 
        ...
        var tiles = L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
            maxZoom: 19,
            attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> | <a href="https://id.wikipedia.org/wiki/Kabupaten_Badung#Pariwisata">Wikipedia di Provinsi Bali Kab/Kota (Pariwisata)</a>'
        }).addTo(map);
        ...
        function getColor(d) {
    return d > 48  ? '#081D58' :   // 9 - Pekat biru
           d > 44  ? '#1E3A8A' :   // 8 - Biru gelap
           d > 40  ? '#2563EB' :   // 7 - Biru terang
           d > 36  ? '#3B82F6' :   // 6
           d > 32  ? '#60A5FA' :   // 5
           d > 28  ? '#93C5FD' :   // 4
           d > 24  ? '#BFDBFE' :   // 3
           d > 20  ? '#DBEAFE' :   // 2
                      '#EFF6FF';   // 1 - Terang biru muda
}

        function style(feature) {
            return {
                fillColor: getColor(feature.properties.pariwisata),
                weight: 2,
                opacity: 1,
                color: 'white',
                dashArray: '3',
        ...
        info.update = function (props) {
            this._div.innerHTML = '<h4>Distribusi Pariwisata di Kab/Kota Bali</h4>' +  (props ?
                '<b>' + props.name + '</b><br />' + props.pariwisata.toLocaleString('id-ID') + ' pariwisata'
                : 'Hover over a state');
        };
        ...
        legend.onAdd = function (map) {
            ...
            var div = L.DomUtil.create('div', 'info legend'),
                grades = [0, 20, 24, 28, 32, 36, 40, 44, 48],
                labels = [];
